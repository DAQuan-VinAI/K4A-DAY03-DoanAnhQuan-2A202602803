...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    # Khởi tạo chuỗi ngữ cảnh tích lũy (Context/History dưới dạng text)
    conversation_context = f"Yêu cầu ban đầu của người dùng: {user_query}"
    ...
    while step < MAX_ITERATIONS:
        ...
        # 📌 Truyền chuỗi ngữ cảnh tích lũy thay vì chuỗi đơn lẻ ban đầu
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if tool_name == "update_student_profile":
                ...
                if confirm != 'y':
                    ...
                    obs_data = {"status": "REJECTED", "message": "Thao tác bị hủy bởi người dùng qua HITL."}
                ...
                # Thực thi Tool qua MCP Server bình thường
                mcp_result = mcp_server.call_tool(tool_name, arguments)
                obs_data = mcp_result.get("result", {})
                
                if not obs_data:
                    print(f"👁️ [Observation từ MCP Server]: {{}}")
                    obs_data = {"status": "EMPTY", "message": "MCP Server trả về kết quả rỗng."}
                    obs_str = json.dumps(obs_data, ensure_ascii=False)
                else:
                    obs_str = json.dumps(obs_data, ensure_ascii=False)
            ...
            # 📌 Cập nhật chuỗi ngữ cảnh tích lũy để vòng lặp sau LLM đọc được kết quả vừa làm
            conversation_context += f"\n- Đã suy luận: {thought}\n- Đã thực thi tool: {tool_name}({arguments})\n- Kết quả nhận được (Observation): {obs_str}"
